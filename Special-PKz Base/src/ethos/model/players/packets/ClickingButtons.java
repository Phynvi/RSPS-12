...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		case 39241:
			...
			c.forcedChat("I've played Special-PKz for a total of : " + time);
			break;
		case 113240:
		...
		case 113243:
			...
			break;
		case 185154: // teleporter - C.T
			c.getPA().showInterface(51000);
			c.getTeleport().selection(c, 0);
			break;
		case 185155: // pk points
			c.sendMessage("Visit our Website, www.Special-PKz.co.uk.");
			break;
		case 185156: // donation amount
			c.setInterfaceOpen(1);
			c.getPresets();
			c.sendMessage("Visit our Website, to view the donation ranks.");
			break;
			
			
			
			
		case 39243: // melee zerker - creator C.T
			if(c.getMode().isUltimateIronman() || c.getMode().isIronman())
			{
				c.getDH().sendItemStatement("You can't do this as an iron man!", 12810);
				c.dialogueAction = 0;
				return;
			}
				if (c.getItems().playerHasItem(995, 300000)) {
					c.getItems().deleteItem(995, 300000);
					c.pkp -= 5;
					QuickSets.gearUp(c, 0);
					c.refreshQuestTab(0);
					c.getDH().sendItemStatement("You become a Melee Zerker.", 22109);
					c.dialogueAction = 0;
				} else {
					c.getDH().sendItemStatement("You need 300k to do this.", 22109);
					c.dialogueAction = 0;
				}
				break;
				
		case 39244: // range tank - creator C.T
			if(c.getMode().isUltimateIronman() || c.getMode().isIronman())
			{
				c.getDH().sendItemStatement("You can't do this as an iron man!", 12810);
				c.dialogueAction = 0;
			...
			if (c.getItems().playerHasItem(995, 300000) && c.pkp >= 5) {
				c.getItems().deleteItem(995, 300000);
				c.pkp -= 5;
				QuickSets.gearUp(c, 1);
				c.refreshQuestTab(0);
				c.getDH().sendItemStatement("You become a Range Tank.", 22109);
				c.dialogueAction = 0;
			} else {
				c.getDH().sendItemStatement("You need 5 PKP and 300k to do this.", 22109);
				c.dialogueAction = 0;
			}
			break;
			
		case 39245: // Brid Main - creator C.T
			if(c.getMode().isUltimateIronman() || c.getMode().isIronman())
			{
				c.getDH().sendItemStatement("You can't do this as an iron man!", 12810);
				c.dialogueAction = 0;
			...
			if (c.getItems().playerHasItem(995, 300000) && c.pkp >= 5) {
				...
				c.refreshQuestTab(0);
				c.getDH().sendItemStatement("You become a Hybrid Main.", 22109);
				c.dialogueAction = 0;
			} else {
				c.getDH().sendItemStatement("You need 5 PKP and 300k to do this.", 22109);
				c.dialogueAction = 0;
		...
		case 39246: // melee pure - creator C.T
			if(c.getMode().isUltimateIronman() || c.getMode().isIronman())
			{
				c.getDH().sendItemStatement("You can't do this as an iron man!", 12810);
				c.dialogueAction = 0;
			...
			if (c.getItems().playerHasItem(995, 300000) && c.pkp >= 5) {
				...
				c.refreshQuestTab(0);
				c.getDH().sendItemStatement("You become a Melee Pure.", 22109);
				c.dialogueAction = 0;
			} else {
				c.getDH().sendItemStatement("You need 5 PKP and 300k to do this.", 22109);
				c.dialogueAction = 0;
		...
		case 39247: // range pure - creator C.T
			if(c.getMode().isUltimateIronman() || c.getMode().isIronman())
			{
				c.getDH().sendItemStatement("You can't do this as an iron man!", 12810);
				c.dialogueAction = 0;
			...
			if (c.getItems().playerHasItem(995, 300000) && c.pkp >= 5) {
				...
				c.refreshQuestTab(0);
				c.getDH().sendItemStatement("You become a Range Pure.", 22109);
				c.dialogueAction = 0;
			} else {
				c.getDH().sendItemStatement("You need 5 PKP and 300k to do this.", 22109);
				c.dialogueAction = 0;
		...
		case 39248: // hybrid pure - creator C.T
			if(c.getMode().isUltimateIronman() || c.getMode().isIronman())
			{
				c.getDH().sendItemStatement("You can't do this as an iron man!", 12810);
				c.dialogueAction = 0;
			...
			if (c.getItems().playerHasItem(995, 300000) && c.pkp >= 5) {
				...
				c.refreshQuestTab(0);
				c.getDH().sendItemStatement("You become a Hybrid Pure.", 22109);
				c.dialogueAction = 0;
			} else {
				c.getDH().sendItemStatement("You need 5 PKP and 300k to do this.", 22109);
				c.dialogueAction = 0;
		...
		case 39249: // f2p pure - creator C.T
			if(c.getMode().isUltimateIronman() || c.getMode().isIronman())
			{
				c.getDH().sendItemStatement("You can't do this as an iron man!", 12810);
				c.dialogueAction = 0;
			...
			if (c.getItems().playerHasItem(995, 300000) && c.pkp >= 5) {
				c.getItems().deleteItem(995, 300000);
				c.pkp -= 5;
				QuickSets.gearUp(c, 6);
				c.refreshQuestTab(0);
				c.getDH().sendItemStatement("You become a Ftp Pure.", 22109);
				c.dialogueAction = 0;
			} else {
				c.getDH().sendItemStatement("You need 5 PKP and 300k to do this.", 22109);
				c.dialogueAction = 0;
			...
			case 185158: // home wilderness tele - creator C.T
				if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
					 c.sendMessage("You can not teleport while above level 20 wilderness.");
					return;
				}
				c.getPA().movePlayer(3275, 3519, 0);
				c.sendMessage("Dangerous Pking Zone, Be safe.");
				break;
			case 185159: // edge wilderness tele - creator C.T
				if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
					 c.sendMessage("You can not teleport while above level 20 wilderness.");
					return;
				}
				c.getPA().movePlayer(3085, 3518, 0);
				c.sendMessage("Edge Pking Zone, Be safe.");
				break;
			case 185160: // green drag wilderness tele - creator C.T
				if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
					 c.sendMessage("You can not teleport while above level 20 wilderness.");
					return;
				}
				c.getPA().movePlayer(3354, 3677, 0);
				c.sendMessage("Green dragons Pking Zone, Be safe.");
				break;
			case 185161: //wilderness event teleport
				if(MonsterHunt.getCurrentLocation() != null) {
					c.getPA().spellTeleport(MonsterHunt.getCurrentLocation().getX(), MonsterHunt.getCurrentLocation().getY(), 0, false);
					}
					c.sendMessage("@blu@[Wilderness Event] @bla@There is no event currently active.");
					break;
			case 185162: // Rev cave pk tele - creator C.T
				if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
					 c.sendMessage("You can not teleport while above level 20 wilderness.");
					return;
				}
				c.getPA().startTeleport(3243, 10166, 0, "modern", false);
				c.sendMessage("Revenant Cave, Dangerous zone, be safe.");
				break;
			case 185142:
		...
		case 33206://attack teleport - C.T
			if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
				c.sendMessage("You can not teleport while above level 20 wilderness.");
				return;
				}
			c.getPA().startTeleport(2673, 3710, 0, "modern", false);
			c.sendMessage("You have been teleported to one of many training grounds.");
			if (c.inClanWarsSafe()) {
		...
		case 33209://strength teleport - C.T
			if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
				c.sendMessage("You can not teleport while above level 20 wilderness.");
				return;
				}
		...
		case 33212://deffence teleport - C.T
			if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
				c.sendMessage("You can not teleport while above level 20 wilderness.");
				return;
				}
		...
		case 33215://Range teleport - C.T
			if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
				c.sendMessage("You can not teleport while above level 20 wilderness.");
				return;
				}
			c.getPA().startTeleport(2524, 4775, 0, "modern", false);
			c.sendMessage("You have been teleported to one of many training grounds.");
			if (c.inClanWarsSafe()) {
				c.outStream.createFrame(27);
				c.rangeSkill = true;
		...
		case 33218://prayer teleport - C.T
			if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
				c.sendMessage("You can not teleport while above level 20 wilderness.");
				return;
				}
			c.getPA().startTeleport(3277, 3513, 0, "modern", false);
			c.sendMessage("Collect bones and use the alter here.");
			if (c.inClanWarsSafe()) {
				c.outStream.createFrame(27);
				c.prayerSkill = true;
			} else {
		...
		case 33221://magic teleport - C.T
			if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
				c.sendMessage("You can not teleport while above level 20 wilderness.");
				return;
				}
			c.getPA().startTeleport(2324, 3802, 0, "modern", false);
			c.sendMessage("You have been teleported to one of many training grounds.");
			if (c.inClanWarsSafe()) {
				c.outStream.createFrame(27);
				c.mageSkill = true;
		...
		case 33210: //agility teleport - C.T
			if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
				c.sendMessage("You can not teleport while above level 20 wilderness.");
				return;
				}
			c.getPA().startTeleport(2481, 3438, 0, "modern", false);
			c.sendMessage("You have been teleported to one of many training grounds.");
			c.getSI().agilityComplex(1);
			c.getSI().selected = 8;
			break;
			
		case 33224: //RuneCraft teleport
			if (c.inWild() && c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
				c.sendMessage("You can not teleport while above level 20 wilderness.");
				return;
				}
		...
		case 75010:
			...
			c.getPA().spellTeleport(3280, 3500, 0, true);
			//c.getPA().showInterface(51000);
			//c.getTeleport().selection(c, 0);
			break;
		case 50056:

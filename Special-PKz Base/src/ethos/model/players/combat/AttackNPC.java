...
//				break;
	...
	public static void attackNpc(Player c, int i) {
		...
		case 8097:
		case 8098:
			if(!npc.galvekAttackable)
			{
				c.sendMessage("Perhaps I should take care of these minions first!.");
				c.getCombat().resetPlayerAttack();
				return;
			}
			break;
		...
		if (NPCHandler.npcs[i] != null) {
			...
			if (c.attackTimer <= 0) {
				...
				case 8097:
				case 8098:
					if(!c.usingBallista && !c.usingCross && !c.usingBow && !c.usingMagic && !c.usingOtherRangeWeapons)
					{
					c.getCombat().resetPlayerAttack();
					c.sendMessage("You can't reach Galvek to harm him with melee!");
					return;
					}
				break;
				}

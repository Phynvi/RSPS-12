...
public class Player extends Entity {
	...
	public long wogwDonationAmount, lastAuthClaim, totalGorillaDamage, totalGorillaHitsDone, totalMissedGorillaHits,
			...
			lastTargeted, homeTeleTime, lastDagChange = -1, reportDelay, lastPlant, poolTimer, objectTimer, npcTimer, lastEss,
	...
	private static final ethos.model.minigames.lighthouse.NylocasVasilias NylocasVasilias = null;
	...
	public void highscores() {
		getPA().sendFrame126("Special-PKz - Top PKers Online", 6399); // Title
		for (int i = 0; i < 10; i++) {
			if (ranks[i] > 0) {
	...
	public void destruct() {
		...
		if (NylocasVasilias != null) {
			InstancedAreaManager.getSingleton().disposeOf(NylocasVasilias);
		}
		...
		if(getMode().isIronman()) {
			com.everythingrs.hiscores.Hiscores.update("AtJ2cd34xE5AXb6qExGmRaMKjnlsntPagPVR6g5EQdH7OSzWtCcuAt9kQihCCuOPGFLxBX1o",  "Iron Man", this.playerName, this.playerRank, this.playerXP, debugMessage);
		}
		if(getMode().isUltimateIronman()) {
			com.everythingrs.hiscores.Hiscores.update("AtJ2cd34xE5AXb6qExGmRaMKjnlsntPagPVR6g5EQdH7OSzWtCcuAt9kQihCCuOPGFLxBX1o",  "Ultimate Iron Man", this.playerName, this.playerRank, this.playerXP, debugMessage);
		}
		if(getMode().isRegular()) {
			com.everythingrs.hiscores.Hiscores.update("AtJ2cd34xE5AXb6qExGmRaMKjnlsntPagPVR6g5EQdH7OSzWtCcuAt9kQihCCuOPGFLxBX1o",  "Normal Mode", this.playerName, this.playerRank, this.playerXP, debugMessage);
		}
	...
	public void updateQuestTab(){
		...
		getPA().sendFrame126("@or1@@cr16@  PK Points = @gre@" +this.pkp,10225);
		...
		getPA().sendFrame126("@or1@@cr16@ Melee Zerker - @gre@set",10227);
		getPA().sendFrame126("@or1@@cr16@ Range Tank - @gre@set",10228);
		getPA().sendFrame126("@or1@@cr16@ Hybrid Main - @gre@set",10229);
		getPA().sendFrame126("@or1@@cr16@ Melee Pure - @gre@set",10230);
		getPA().sendFrame126("@or1@@cr16@ Range Pure - @gre@set",10231);
		getPA().sendFrame126("@or1@@cr16@ Hybrid Pure - @gre@set",10232);
		getPA().sendFrame126("@or1@@cr16@ F2p Pure - @gre@set",10233);


		getPA().sendFrame126("@or1@Teleport Menu",47514);
		getPA().sendFrame126("@or1@@cr20@Time Played = @gre@"+time,47515);
		getPA().sendFrame126("@or1@@cr8@ Donation Amount = @gre@$" + this.amDonated,47516);
		getPA().sendFrame126("@or1@@cr1@ Player Rank = @gre@"+getRights().getPrimary().toString(),47517);
		getPA().sendFrame126("@or1@@cr15@ Player Title = @gre@ ",47518);
		getPA().sendFrame126("@or1@@cr21@ Wilderness Teleport - @red@edge",47519);
		getPA().sendFrame126("@or1@@cr21@ Green Dragons Teleport - @red@east",47520);
		getPA().sendFrame126("@or1@@cr21@ Wilderness Event - @red@Teleport",47521);
		getPA().sendFrame126("@or1@@cr21@ Revenant Cave - @red@danger zone",47522);
	...
	public NylocasVasilias getNylocasVasilias() {
		return NylocasVasilias;
	}
	...
	public NylocasVasilias createNylocasVasiliasInstance() {
		Boundary boundary = Boundary.LIGHTHOUSE;
	...
	public boolean inMulti() {
		if (Boundary.isIn(this, Boundary.GALVEK_LAIR) || Boundary.isIn(this, Zulrah.BOUNDARY) || Boundary.isIn(this, Boundary.CORPOREAL_BEAST_LAIR)
